...
options:
  ...
  force_basic_auth:
    description:
      - Force sending of the Basic authentication header upon initial request.
    type: bool
    default: false
  ca_path:
    description:
...
def main() -> None:
    ...
    client = Icinga2Client(module=module, url=module.params["url"], timeout=module.params.get("timeout"))
...
def schedule_downtime(module: AnsibleModule, client: Icinga2Client) -> None:
    ...
    if 200 <= status_code <= 299:
        result["changed"] = True
        result["msg"] = "Successfully scheduled downtime."
        try:
...
def remove_downtime(module: AnsibleModule, client: Icinga2Client) -> None:
    ...
    if 200 <= status_code <= 299:
        result["changed"] = True
        result["msg"] = "Successfully removed downtime."
        try:
            result["results"] = module.from_json(response.read())["results"]
        except (ValueError, KeyError):
            # As a precaution, catch key and value error in case of a malformed response message.
